...
4. `export_plots_training.ipynb` — turns the raw `training/*_training/` logs into the
   paper's training-curve figures (GP/DQN/LSPI/PLA), saved under `results/plots/`. It also
   includes a Monte Carlo convergence check for the exact value function policy: it runs
   many episodes of the greedy-on-V policy per `(facilities, capacity distribution)`
   instance and plots the running-average episode reward against the true `V(state_0)`
   baseline from `train_exact_value_function.py`, saved under
   `training/exact_value_function_training/plots/`. Episodes are generated by re-seeding
   `np.random` per episode (incrementing across the whole replication × episode grid, never
   reset) rather than from pre-generated instance files, so it stays fully reproducible
   without needing `instances_extra/`.
...
- `instances_extra/` — additional instances used by `export_plots_results.ipynb` for the
...
`point_estimate_lookahead_policy.py` approximates unseen demand by Monte Carlo sampling
`num_samples` synthetic instances (i.i.d. uniform customer locations) around the real
current customer, re-solving the assignment LP for each and taking a majority vote over
the resulting facility choice. `exact_value_function_policy.py` instead acts greedily on
a post-decision value function solved exactly offline by backward induction (see
`train_exact_value_function.py` below) — capacity remaining per facility is the only
state, customer location only enters the immediate reward.
